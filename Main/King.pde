public class King extends ChessPieceClass {
  ...
  boolean movement(int newX, int newY){
    ...
    if(board.getChessPiece(newX, newY) == null || team != board.getChessPiece(newX, newY).getTeam()){
      ...
      //Small castle.
      if(newX == 6 && position[0] == 4 && !hasMoved && board.getChessPiece(7, position[1]) != null && board.getChessPiece(7, position[1]).getType() == "Rook" && !board.getChessPiece(7, position[1]).getHasMoved()){
        ...
        if(canCastle){
          board.setChessPiece(7, position[1], 5, position[1]); //Puts the picked up piece into the cell that matches its new spot.
          board.getChessPiece(7, position[1]).setPosition(5, position[1]); //Sets the piece position inside the piece object.
          board.removeChessPiece(7, position[1]); //Remove the pointer to the piece in the previous spot.
      ...
      //Long castle.
      if(newX == 2 && position[0] == 4 && !hasMoved && board.getChessPiece(0, position[1]) != null && board.getChessPiece(0, position[1]).getType() == "Rook" && !board.getChessPiece(0, position[1]).getHasMoved()){
        ...
        if(canCastle){
          board.setChessPiece(0, position[1], 3, position[1]); //Puts the picked up piece into the cell that matches its new spot.
          board.getChessPiece(0, position[1]).setPosition(3, position[1]); //Sets the piece position inside the piece object.
